...
10月6日: 
* yingshaoxo: 去学习(把OpenMV当成)`光流(Optical Flow Senser)`的用法 | 未完成，缺`特制的的飞控线转杜邦线`; 配置好`raspberry pie 3B+`，接下来打算把它送上天
* 孙雅斌: 和淘宝店家沟通，索要更多资料
* 刘露: 在淘宝买`飞控线转杜邦线`
...
### Task4: 以时间先后顺序辨别1、2、3电杆； 用程序测算`从起点 到 色块 之间的距离`（假设速度恒定，我们需要得到经过的时间）； 利用OpenMV测算`第2根杆 与 第3根杆 的夹角度数` (即`第2杆消失，第3杆出现，无人机所旋转的角度`)
