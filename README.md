...
### Task4: 通过`模拟器`学习如何让无人机在原地旋转指定的度数、如何让无人机向各个方向`飞行指定的距离`； 硬件上把由三根黑杆组成的场地搭好
...
10月10日: 
* yingshaoxo: 学会了如何让无人机在原地旋转指定的度数、如何让无人机向各个方向`飞行指定的距离`。(换句话讲，知道了两种控制方法: `velocity+time` and `relative_coordinate + clockwise_angle`)
* 孙雅斌: 想到了用哑铃片来固定黑色杆子的idea
* 刘露: 无

___

### Task5: 依靠实际场地，写出完整的摄像头程序: 以时间先后顺序辨别1、2、3电杆； 用程序测算`从起点 到 色块 之间的距离`（假设速度恒定，我们需要得到经过的时间）； 利用OpenMV测算`第2根杆 与 第3根杆 的夹角度数` (即`第2杆消失，第3杆出现，无人机所旋转的角度`)；要求能让摄像头完成从`第一根黑杆`到`第三根黑杆`的`全部飞行指导任务`; 硬件上要把所有传感器(`光流、摄像头、树梅派、激光`)都装在飞机上

10月11日: 
* yingshaoxo: 用时间间隔的方法区分出了3根杆; 彩块距离需飞控回传 Local Location, 不是摄像头能做的
* 孙雅斌: 无
* 刘露: 无
